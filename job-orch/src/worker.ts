...
async function claimNextJob() {
  ...
  try {
    ...
    // skip it instead of waiting — that's what makes this safe to
    // run with multiple worker instances at once.
...
async function processJob(job: any) {
  ...
  try {
    ...
    await pool.query(
      `UPDATE jobs
       SET provider_job_id = $1, attempts = attempts + 1, updated_at = now()
       WHERE id = $2`,
...
async function pollLoop() {
  ...
  if (job) {
    ...
    setImmediate(pollLoop); // more work might be waiting, don't sleep
  } else {
